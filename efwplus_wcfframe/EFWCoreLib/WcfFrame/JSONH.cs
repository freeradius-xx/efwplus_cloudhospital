...
namespace EFWCoreLib.WcfFrame
{
    public class JSONH
    {
        ...
        static public int best(List<Dictionary<string, object>> collection)
        {
            ...
            for (int i = 0, len = 0, length = 0; i < 4; i++)
            {
                ...
                len = JsonConvert.SerializeObject(_cache[i]).Length;
                if (length == 0)
                    length = len;
                else if (len < length)
                {
                ...
                if (2 < compression)
                {
                    for (int j = 0; j < len; ++j)
                    {
                        if (header[j] is List<object>)
                        {
                            ...
                            if (JsonConvert.SerializeObject(values).Length < JsonConvert.SerializeObject(indexes).Length)
                            {
    ...
    public class JsonComporessor
    {
        ...
        {
            List<Dictionary<string, object>> unpacked = JsonConvert.DeserializeObject<List<Dictionary<string, object>>>(input);
            return JsonConvert.SerializeObject(JSONH.pack(unpacked, 0));
        }
        ...
        {
            List<List<object>> packed = JsonConvert.DeserializeObject<List<List<object>>>(input);
            return JsonConvert.SerializeObject(JSONH.unpack(packed));
        }
    }
}
